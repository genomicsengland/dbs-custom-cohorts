...
csv_path = r'./raw_data/Severe_To_Trace.csv'
...
prep_dict['local_pid'] = list(df['genomicc_id'])
prep_dict['date_of_birth'] = list(df['date_of_birth'])

prep_dict['sex'] = list(df['sex'])
prep_dict['pcd_postcode'] = list(df['postcode'])



names = df['name']
fname = [x.split(' ')[0].upper() for x in names]
sname = [x.split(' ')[1].upper() for x in names]

prep_dict['surname'] = sname
prep_dict['forename'] = fname


df_out = pd.DataFrame(prep_dict)

df_out.to_csv('./request_data/severe_request_file.csv', index=False)
